...
export default function AdminChartPanel() {
  ...
  const [revenueDate, setRevenueDate] = useState({ year: NOW.getFullYear(), month: NOW.getMonth() + 1 });
  const [regionDate, setRegionDate] = useState({ year: NOW.getFullYear(), month: NOW.getMonth() + 1 });
  const [cumulativeDate, setCumulativeDate] = useState({ year: NOW.getFullYear(), month: NOW.getMonth() + 1 });
  const [regionModalOpen, setRegionModalOpen] = useState(false);
  const [modalDate, setModalDate] = useState({ year: NOW.getFullYear(), month: NOW.getMonth() + 1 });

  // 백엔드 통계 데이터용 State
  const [graphData, setGraphData] = useState({ sixMonths: [], twelveMonths: [] });
  ...
  useEffect(() => {
    async function fetchMonthlySales() {
      try {
        const resp = await getMonthlyRegionStats(revenueDate.year, revenueDate.month);
  ...
  useEffect(() => {
    async function fetchRegionStats() {
      try {
        ...
        const resp = await getMonthlyRegionStats(regionDate.year, regionDate.month);
        const list = resp.data?.areaSalesList || [];
        const sorted = [...list].sort((a, b) => b.totalNetSales - a.totalNetSales);
  ...
  useEffect(() => {
    ...
    async function fetchModalRegionStats() {
      try {
        ...
        const resp = await getMonthlyRegionStats(modalDate.year, modalDate.month);
        const list = resp.data?.areaSalesList || [];
        const sorted = [...list].sort((a, b) => b.totalNetSales - a.totalNetSales);
    ...
    <Grid>
      ...
      <ChartCard>
        <ChartHeader>
          ...
          <PeriodToggle>
            <PeriodBtn $active={period === '6m'} onClick={() => setPeriod('6m')}>
              최근 6개월
            </PeriodBtn>
            <PeriodBtn $active={period === '1y'} onClick={() => setPeriod('1y')}>
              최근 1년
            </PeriodBtn>
          </PeriodToggle>
        </ChartHeader>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <RechartsBarChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  formatter={(value) => [`₩${value.toLocaleString()}`, '매출액']}
                  contentStyle={{ background: '#0d1c2e', border: 'none', borderRadius: '8px', padding: '10px' }}
      ...
      <RegionCard>
        ...
        <RegionList>
          {regionLoading ? (
            <LoadingText style={{ height: '140px', position: 'relative' }}>데이터 조회 중...</LoadingText>
          ) : regionalSales.length === 0 ? (
            <LoadingText style={{ height: '140px', position: 'relative' }}>매출 정보가 없습니다.</LoadingText>
          ) : (
            regionalSales.map((item, idx) => (
              <RegionItem key={item.areaName}>
                ...
                <RegionAmount>₩{item.totalNetSales.toLocaleString()}</RegionAmount>
              </RegionItem>
            ))
          )}
        </RegionList>
        ...
        <RegionFooter>
          <RegionAllBtn onClick={() => setRegionModalOpen(true)}>전체 순위 보기 &gt;</RegionAllBtn>
        </RegionFooter>
      </RegionCard>
    </Grid>

    {regionModalOpen && createPortal(
                ...
                {modalRegionLoading ? (
                  ...
                  modalRegionalSales.map((item, idx) => (
                    <ModalTR key={item.areaName}>
                      ...
                      <ModalTD><RegionNameCell>{item.areaName}</RegionNameCell></ModalTD>
                      <ModalTD $right><AmountCell>₩{item.totalNetSales.toLocaleString()}</AmountCell></ModalTD>
                    </ModalTR>
                  ))
                )}
              </ModalTBody>
